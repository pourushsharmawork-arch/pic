...
| lossless + powers | `power` | `1.0` | 32 → 24 (12 angles) | **no** |
...
| power | 48 | 32 | I1…I4 + I12,I23,I34,I14 |
| power + quadrature | **64** | **48** | above + I12q,I23q,I34q,I14q |

NPZ files also store `observation`, `loss`, and `quadrature` metadata keys
(when generated by `datagen.py`). The loader auto-detects quadrature from
64-column rows for power mode.
...
| `run_regime.py` | CLI for all four regimes |
...
Useful flags: `--n_starts 8`, `--n_refine_eval 200`, `--quadrature` (power
only), `--mode param_only` (ablation), `--skip_convention_check`, `--out my_model.pt`.
