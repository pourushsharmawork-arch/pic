...
def _load_i2_i3_row(path: str, index: int) -> tuple[torch.Tensor, np.ndarray]:
    ...
    row = rows[index]
    return torch.from_numpy(row[16:24]).unsqueeze(0), row[:16]
...
def _print_observation(label: str, predicted: torch.Tensor, target: torch.Tensor) -> None:
    ...
    print(f"\n[{label} observation]")
    for probe_index, probe_name in enumerate(("I2", "I3")):
        start = probe_index * 4
...
def main() -> None:
    ...
    parser.add_argument(
        "--data",
        type=Path,
        default=Path("NEW/training_power_data_I2_I3.npz"),
    )
    ...
    regime = I2_I3_REGIME
    obs, true_params = _load_i2_i3_row(str(args.data), args.index)
    ...
    print("[inference]")
    print("  regime       : power, custom probes I2 + I3")
    ...
    _print_observation("network", predicted_obs0, obs)
    _print_observation("refined", predicted_obs, obs)
